...
function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      const totalIncome = items.reduce((total, item) => total + (item.incomePerHour / 3600), 0); // Правильный расчет дохода в секунду
      setPoints(prevPoints => {
        const newPoints = prevPoints + Math.floor(totalIncome); // Обновляем на основе текущих очков
